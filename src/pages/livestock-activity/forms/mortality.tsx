...
export default function MortalityPage() {
  ...
  useEffect(() => {
    setInitLoading(true);
    const promises = [];
    if (!(eventTypes.length > 0 && eventTypes[0].Journal_Template_Name == "MORTALITY")) promises.push(getEventTypes("MORTALITY"));
  ...
  return (
    <>
      ...
      {!initLoading && (
        <>
          <CustomNotice<MortalityFormData> formType={MORTALITY_STORAGE_KEY} onLoad={(data) => reset(data)} />
          <CustomFormsLayout>
            <CustomHeader icon={ReportProblem} title="Mortality" button={{ label: "reset", onClick: handleReset }} />
