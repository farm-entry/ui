...
export const JobSelection: React.FC<JobSelectionProps> = ({ config, control, errors, jobs, selectedJob }) => {
  ...
  return (
    <Box>
      ...
      {config.sections.jobSelection.showFromJob && config.sections.jobSelection.showToJob ? (
        <Stack direction={{ xs: "column", md: "row" }} spacing={2}>
          <Controller
            ...
            render={({ field, fieldState }) => {
              const selectedValue = jobs.find((job) => job.number === field.value) || null;
              ...
              return (
                <TypeAhead
                  label={config.sections.jobSelection.fromJobLabel || "From Group"}
                  options={jobOptions}
                  value={valueAsOption}
          ...
          <Controller
            name="toJob"
            control={control}
            rules={{ required: "To job is required" }}
            render={({ field, fieldState }) => {
              const selectedValue = jobs.find((job) => job.number === field.value) || null;
              const valueAsOption: TypeAheadOption | null = selectedValue
                ? {
              ...
              return (
                <TypeAhead
                  label={config.sections.jobSelection.toJobLabel || "To Group"}
                  options={jobOptions}
                  value={valueAsOption}
                  onChange={(value) => field.onChange(value?.value || "")}
                  helperText={fieldState.error?.message}
        ...
        <Controller
          ...
          render={({ field, fieldState }) => {
            ...
            return (
              <TypeAhead
                label={config.sections.jobSelection.jobLabel || "Select Job/Group"}
                options={jobOptions}
                value={valueAsOption}
                onChange={(value) => field.onChange(value?.value || "")}
                helperText={fieldState.error?.message}
          ...
          <Stack direction={{ xs: "column", sm: "row" }} spacing={2}>
            ...
            {selectedJob.healthStatus && (
              <Chip
                ...
                size="small"
                color={selectedJob.healthStatus.color === "green" ? "success" : selectedJob.healthStatus.color === "red" ? "error" : "warning"}
              />
            )}
          </Stack>
        </Box>
